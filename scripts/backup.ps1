...
$migration = (docker compose -f compose.dev.yml exec -T postgres psql -U firebot -d firebot -Atc 'SELECT version_num FROM alembic_version;').Trim()
...
$partitions = docker compose -f compose.dev.yml exec -T postgres psql -U firebot -d firebot -Atc $partitionSql
...
$defaultRows = docker compose -f compose.dev.yml exec -T postgres psql -U firebot -d firebot -Atc $defaultSql
$manifest = @{
