...
def test_manifest_freezes_contract_and_topics() -> None:
    manifest = json.loads((HANDOFF / "ROBOT_INTEGRATION_MANIFEST.json").read_text(encoding="utf-8"))
...
def test_handoff_schema_is_canonical() -> None:
    ...
    delivered = HANDOFF / "schemas/firebot-message-1.2.schema.json"
    assert (
...
def test_invalid_schema_vector_is_rejected() -> None:
    payload = json.loads((HANDOFF / "test-vectors/invalid_schema.json").read_text(encoding="utf-8"))
...
def test_duplicate_vector_keeps_identical_command_id_and_payload() -> None:
    first = json.loads(
        (HANDOFF / "test-vectors/duplicate_command_first.json").read_text(encoding="utf-8")
    )
    retry = json.loads(
        (HANDOFF / "test-vectors/duplicate_command_retry.json").read_text(encoding="utf-8")
    )
    assert first == retry
