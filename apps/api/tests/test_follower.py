...
def test_large_heading_error_rotates_in_place_without_driving() -> None:
    ...
    assert linear == 0.0
    assert angular > 0
    assert abs(heading_error) > 0.14


def test_aligned_segment_drives_at_distance_speed() -> None:
    dx, dy = 0.0, -10.0  # straight south
    linear, _angular, state, _heading = follower_command(-math.pi / 2, dx, dy, 10.0)
    assert state == "DRIVE"
    assert linear > 0
...
def test_arrive_snaps_and_returns_zero_command() -> None:
    linear, angular, state, _heading = follower_command(0.0, 0.1, 0.0, 0.1)
    assert state == "ARRIVE"
    assert linear == 0.0
    assert angular == 0.0
