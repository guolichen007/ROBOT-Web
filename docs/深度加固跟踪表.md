...
| AUDIT-58 | PASS | commercial HMI | 首页硬编码 R001/道路、时钟不响应、同级报警抢焦点、低分辨率裁切和后台多路视频连接会破坏现场可用性。 | 引入 active robot、Sticky Primary Alarm、语义地图、独立 viewport、单 active WebRTC、风险带、生命周期/时间线及 1366/1440/1920 响应式工业 HMI。 |
| AUDIT-59 | PASS | monitor bootstrap | 根组件在登录前已经 mounted，登录跳转不会再次触发 snapshot 启动；`0-0` watermark 又被误当成已加载，首页长期显示 NO ROBOT。 | Monitor 页按连接/同步状态幂等启动 store；实际浏览器从登录跳转后可获得 R001 snapshot 并连接一次性 ticket WebSocket。 |
| AUDIT-60 | PASS | WebSocket idle | `XREAD BLOCK 5000ms` 复用 `socket_timeout=2s` 的 Redis client，事件流正常空闲时会超时并触发浏览器永久重连。 | 将阻塞窗限制为 1 秒，在 socket timeout 前返回并发送 heartbeat；实际浏览器链路空闲后保持“链路正常”。 |
| AUDIT-61 | PASS | map interaction | SVG viewport 在车位子节点 pointerdown 时执行 pointer capture，真实 Chromium 会把后续 click 重定向到父容器，导致车位看似选中但 Selection Bar/人工上报无法出现。 | 交互节点独立拥有 pointer sequence，只有空白地图进入 pan capture；A-12 选择→人工火情 Playwright 闭环通过。 |
| AUDIT-62 | PASS | HMI functional parity | 商业 HMI 初版遗漏原有“手动控制（高级操作）”入口，且租约提示仍硬编码 R001。 | 恢复抽屉但不恢复速度调节；复用 150ms pulse、500ms TTL、hidden/blur stop+release，车辆标识绑定 active robot；租约互斥与隐藏释放 E2E 通过。 |
| AUDIT-63 | PASS | alarm timeline | 命令时间线若只在 `issued_at` 展示当前最终 lifecycle，会把未来的发布/ACK/终态伪装成创建时已经发生。 | 分别按 `issued_at/published_at/ack_at/terminal_at` 生成 CREATED/PUBLISHED/ACK/terminal 事件，保持后端事实时间语义。 |
